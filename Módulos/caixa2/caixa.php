...
<div>
<?php echo number_format($p['preco_venda'],2,',','.'); ?>
</div>
...
'<?php echo $p['id']; ?>',
'<?php echo $p['nome']; ?>',
<?php echo $p['preco_venda']; ?>,
<?php echo $p['preco_custo']; ?>
)">
+
</button>

<button onclick="removerCarrinho('<?php echo $p['id']; ?>')">
-
</button>
...
<div>
Total:
<b id="total1">R$ 0,00</b>
</div>

<button class="btn ok" onclick="irTela2()">
Finalizar Pedido
</button>
...
let comissao=total*0.10;

if(this.value!=""){

document.getElementById('comissao-func').innerHTML=
"Comissão do funcionário: <b>"+dinheiro(comissao)+"</b>";
...
.addEventListener('change',calcularParcelas);
...
document.getElementById('valor_pago')
...
if(
(forma==="Dinheiro" || forma==="Cheque")
&& pago < total
){

alert("Dinheiro insuficiente para finalizar a venda");
...
<b>Total:</b> ${dinheiro(total)}<br>

<b>Funcionário:</b> ${funcionario}<br>

<b>Comissão:</b> ${dinheiro(comissao)}
...
document.getElementById('f_investido').innerText =
dinheiro(totalInvestido);

document.getElementById('f_meta').innerText =
dinheiro(meta);

document.getElementById('f_total').innerText =
dinheiro(caixa);

if(resultado >= 0){

document.getElementById('f_resultado').innerText =
"Lucro: " + dinheiro(resultado);

}else{

document.getElementById('f_resultado').innerText =
"Prejuízo: " + dinheiro(Math.abs(resultado));
